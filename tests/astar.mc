...
IF(config.astar.debug.functions) {
	dir debug {
		...
		function clear_path_que {
			data modify storage astar:ram path_que set value []
		}
		...
		function reset {
			...
			function astar:debug/clear_path_que
...
function que_path {
	...
	# Add this new path to the path_que
	data modify storage astar:ram path_que append from storage astar:ram new.path
...
dir internal {
	...
	clock 1t {
		name tick
		# Count the number of paths in the que
		execute store result score #count aS.v run data get storage astar:ram path_que
	...
	dir search {
		function start {
			...
			# Get the next path in the que
			data modify storage astar:ram this.path set from storage astar:ram path_que[0]
			data remove storage astar:ram path_que[0]
			...
			IF(config.astar.debug.verbose){
				log info json [{"text":"Solving next path in que","color":"yellow"}]
		...
		function solve_lowest_h_cost {
			...
			# If there is only one node with lowest G cost under F cost
		...
		function complete_intermidate {
			...
			execute (if data storage astar:ram {this:{path:{can_return_intermediate:1b}}}) {
				data modify storage astar:ram this.path.is_intermediate set value 1b
				function astar:internal/spread/complete_search
			} else {
				data modify storage astar:ram this.path.is_intermediate set value 1b
				function astar:internal/search/fail_search
			}
